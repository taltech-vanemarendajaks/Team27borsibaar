...
export default function Inventory() {
  ...
  const fetchTransactionHistory = async (productId: number) => {
    try {
      ...
      const response = await fetch(`/api/backend/inventory/product/${productId}/history`, {
        credentials: "include",
      });
  ...
  const handleCreateProduct = async () => {
    try {
      ...
      if (productForm.initialQuantity && parseFloat(productForm.initialQuantity) > 0) {
  ...
  const getStockStatus = (quantity) => {
    ...
    if (qty === 0) return { color: "text-red-100", bg: "bg-red-900", label: "Out of Stock" };
    if (qty < 10)
      return {
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-400">
                ...
                <th className="text-left py-3 px-4 font-semibold text-gray-300">Current Price</th>
                <th className="text-left py-3 px-4 font-semibold text-gray-300">Min Price</th>
                <th className="text-left py-3 px-4 font-semibold text-gray-300">Max Price</th>
                <th className="text-center py-3 px-4 font-semibold text-gray-300">Quantity</th>
            ...
            <tbody>
              {filteredInventory.length === 0 ? (
                ...
                filteredInventory.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="border-b border-gray-400 hover:bg-gray-800">
                      <td className="py-3 px-4">
                        <div className="font-medium text-gray-300">{item.productName}</div>
                        <div className="text-sm text-gray-400">ID: {item.productId}</div>
                      </td>
          ...
          <div className="space-y-4 mt-4">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="0"
              />
              <p className="text-xs text-gray-500 mt-1">Leave empty or 0 for no initial stock</p>
            </div>
            <div>
      ...
      <Dialog open={showDeleteProductModal} onOpenChange={setShowDeleteProductModal}>
        <DialogContent className="sm:max-w-[480px]">
          <DialogHeader>
            ...
            <DialogDescription>
              This action will permanently delete the product and its related data. Are you sure you
              want to continue?
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="mt-4">
            ...
            <p className="text-sm text-gray-400 mt-2">
              ID:{" "}
              <span className="font-mono">{selectedProduct?.productId ?? selectedProduct?.id}</span>
            </p>
          </div>
                ...
                {transactionHistory.map((transaction) => (
                  ...
                  >
                    ...
                    {transaction.referenceId && (
                      <p className="text-xs text-gray-500 mt-1">Ref: {transaction.referenceId}</p>
                    )}
                    {(transaction.createdByName || transaction.createdByEmail) && (
                      <div className="flex items-center gap-1 mt-2 text-xs text-gray-400">
                        <User className="w-3 h-3" />
                        <span>By: {transaction.createdByName || transaction.createdByEmail}</span>
                      </div>
                    )}
                  </div>
                ))}
