...
public class InventoryService {
  ...
  public List<InventoryResponseDto> getByOrganization(Long organizationId, Long categoryId) {
    ...
    if (categoryId != null) {
      inventories =
          inventoryRepository.findByOrganizationIdAndCategoryId(organizationId, categoryId);
    } else {
        ...
        .map(
            inv -> {
              ...
              BigDecimal unitPrice =
                  Optional.ofNullable(inv.getAdjustedPrice()).orElse(product.getBasePrice());
  ...
  public InventoryResponseDto getByProductAndOrganization(Long productId, Long organizationId) {
    ...
    String productName = product.getName();
    BigDecimal unitPrice =
        Optional.ofNullable(inventory.getAdjustedPrice()).orElse(product.getBasePrice());
  ...
  public InventoryResponseDto addStock(
      AddStockRequestDto request, UUID userId, Long organizationId) {
  ...
  public InventoryResponseDto removeStock(
      RemoveStockRequestDto request, UUID userId, Long organizationId) {
  ...
  public InventoryResponseDto adjustStock(
      AdjustStockRequestDto request, UUID userId, Long organizationId) {
    Product product = getOrganizationProduct(organizationId, request.productId());
  ...
  public List<InventoryTransactionResponseDto> getTransactionHistory(
      Long productId, Long organizationId) {
    Inventory inventory =
        inventoryRepository
            .findByOrganizationIdAndProductId(organizationId, productId)
            .orElseThrow(
    ...
    List<InventoryTransaction> transactions =
        inventoryTransactionRepository.findByInventoryIdOrderByCreatedAtDesc(inventory.getId());
            ...
            transaction -> {
              ...
              return new InventoryTransactionResponseDto(
                  ...
                  transaction.getCreatedAt() != null
                      ? transaction.getCreatedAt().format(DateTimeFormatter.ISO_OFFSET_DATE_TIME)
                      : null);
            })
        .toList();
  }
  ...
  public List<UserSalesStatsResponseDto> getUserSalesStats(Long organizationId) {
    ...
    List<InventoryTransaction> saleTransactions =
        inventoryTransactionRepository.findSaleTransactionsByOrganizationId(organizationId);

    // Group transactions by user and station
    Map<String, List<InventoryTransaction>> transactionsByUserAndStation =
        saleTransactions.stream()
    ...
    Map<Long, BarStation> stationMap =
        barStationRepository.findAllById(new ArrayList<>(stationIds)).stream()
    ...
    return transactionsByUserAndStation.entrySet().stream()
        .map(
            entry -> {
              ...
              long salesCount =
                  userStationTransactions.stream()
  ...
  public List<StationSalesStatsResponseDto> getStationSalesStats(Long organizationId) {
    ...
    Map<Long, List<InventoryTransaction>> transactionsByStation =
        saleTransactions.stream()
        ...
        .map(
            entry -> {
              ...
              return new StationSalesStatsResponseDto(
                  stationId, station != null ? station.getName() : null, salesCount, totalRevenue);
            })
        .sorted((a, b) -> Long.compare(b.salesCount(), a.salesCount())) // Sort by sales count
        // desc
        .toList();
