...
public class BarStationService {
  ...
  public BarStationResponseDto getStationById(Long organizationId, Long stationId) {
    BarStation station =
        barStationRepository
            .findByOrganizationIdAndId(organizationId, stationId)
  ...
  public BarStationResponseDto createStation(Long organizationId, BarStationRequestDto request) {
    ...
    boolean nameExists =
        existingStations.stream().anyMatch(s -> s.getName().equalsIgnoreCase(request.name()));
    if (nameExists) {
  ...
  public BarStationResponseDto updateStation(
      Long organizationId, Long stationId, BarStationRequestDto request) {
    BarStation station =
        barStationRepository
            .findByOrganizationIdAndId(organizationId, stationId)
            .orElseThrow(() -> new NotFoundException("Bar station not found"));
    ...
    List<BarStation> existingStations = barStationRepository.findByOrganizationId(organizationId);
    boolean nameExists =
        existingStations.stream()
            .anyMatch(
                s -> !s.getId().equals(stationId) && s.getName().equalsIgnoreCase(request.name()));
    if (nameExists) {
      throw new DuplicateResourceException("A bar station with this name already exists");
    }
  ...
  private Set<User> assignUsersToStation(
      Long organizationId, List<UUID> userIds, BarStation station) {
