...
class JwtAuthenticationFilterTest {
  ...
  void testFilter_WithValidJwtCookie_AuthenticatesUser() throws Exception {
    ...
    mockMvc.perform(get("/api/users").cookie(new Cookie("jwt", token))).andExpect(status().isOk());
  }
  ...
  void testFilter_WithInvalidJwtCookie_DoesNotAuthenticate() throws Exception {
    ...
    mockMvc
        .perform(get("/api/users").cookie(new Cookie("jwt", invalidToken)))
  ...
  void testFilter_WithValidTokenButUserNotFound_DoesNotAuthenticate() throws Exception {
    ...
    when(userRepository.findByEmailWithRole(anyString())).thenReturn(Optional.empty());
    ...
    // (302)
    mockMvc
        .perform(get("/api/users").cookie(new Cookie("jwt", token)))
        .andExpect(status().is3xxRedirection());
  }
  ...
  void testFilter_WithExpiredToken_DoesNotAuthenticate() throws Exception {
    ...
    mockMvc
        .perform(get("/api/users").cookie(new Cookie("jwt", expiredToken)))
        .andExpect(status().is3xxRedirection());
  }
  ...
  void testFilter_WithEmptyJwtCookie_DoesNotAuthenticate() throws Exception {
    ...
    mockMvc
        .perform(get("/api/users").cookie(new Cookie("jwt", "")))
        .andExpect(status().is3xxRedirection());
  }
  ...
  void testFilter_AllowsAuthLogoutWithJwt() throws Exception {
    ...
    mockMvc
        .perform(get("/auth/logout").cookie(new Cookie("jwt", token)))
