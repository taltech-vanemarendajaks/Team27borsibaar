...
class UserControllerTest {
  ...
  void testGetOrganizationUsers_AsAdmin_ReturnsUsers() throws Exception {
    ...
    UserSummaryResponseDto dto1 =
        new UserSummaryResponseDto(user1.getId(), user1.getEmail(), user1.getName(), "ADMIN");
    UserSummaryResponseDto dto2 =
        new UserSummaryResponseDto(user2.getId(), user2.getEmail(), user2.getName(), "ADMIN");
  ...
  /**
   * Helper method to setup SecurityContext with a mock authenticated user. This simulates the
   * authentication that would normally be set by JwtAuthenticationFilter.
   *
