...
class SalesControllerTest {
  ...
  void processSale_ReturnsCreatedResponse() throws Exception {
    ...
    SaleRequestDto req =
        new SaleRequestDto(List.of(new SaleItemRequestDto(10L, new BigDecimal("2"))), "note", 5L);
    SaleItemResponseDto itemResp =
        new SaleItemResponseDto(
            10L, "Cola", new BigDecimal("2"), new BigDecimal("3.00"), new BigDecimal("6.00"));
    SaleResponseDto resp =
        new SaleResponseDto(
            "SALE-1", List.of(itemResp), new BigDecimal("6.00"), "note", OffsetDateTime.now());
    when(salesService.processSale(any(SaleRequestDto.class), any(UUID.class), anyLong()))
        .thenReturn(resp);
