...
class InventoryControllerTest {
  ...
  void getOrganizationInventory_UsesQueryParams_WhenProvided() throws Exception {
    when(inventoryService.getByOrganization(99L, 7L))
        .thenReturn(
            List.of(
                new InventoryResponseDto(
                    1L,
    ...
    mockMvc
        .perform(get("/api/inventory").param("organizationId", "99").param("categoryId", "7"))
  ...
  void getProductInventory_DelegatesToService() throws Exception {
    ...
    setAuth(user);
    when(inventoryService.getByProductAndOrganization(10L, 5L))
        .thenReturn(
            new InventoryResponseDto(
                1L,
                5L,
    ...
    mockMvc
        .perform(get("/api/inventory/product/{productId}", 10L))
        .andExpect(status().isOk())
  ...
  void getTransactionHistory_ReturnsList() throws Exception {
    ...
    setAuth(user);
    when(inventoryService.getTransactionHistory(40L, 4L))
        .thenReturn(
            List.of(
    ...
    mockMvc
        .perform(get("/api/inventory/product/{productId}/history", 40L))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$", hasSize(1)));
  ...
  void getUserSalesStats_ReturnsList() throws Exception {
    ...
    setAuth(user);
    when(inventoryService.getUserSalesStats(6L))
        .thenReturn(
            List.of(
                new UserSalesStatsResponseDto(
                    UUID.randomUUID().toString(),
                    "U",
                    "u@x",
                    2L,
    ...
    mockMvc
        .perform(get("/api/inventory/sales-stats"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$", hasSize(1)));
  ...
  void getStationSalesStats_ReturnsList() throws Exception {
    ...
    setAuth(user);
    when(inventoryService.getStationSalesStats(7L))
        .thenReturn(
            List.of(new StationSalesStatsResponseDto(1L, "Main", 3L, new BigDecimal("30.00"))));

    mockMvc
        .perform(get("/api/inventory/station-sales-stats"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$", hasSize(1)));
