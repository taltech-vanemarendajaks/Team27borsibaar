...
class BarStationControllerTest {
  ...
  @MockitoBean private BarStationService barStationService;
  ...
  void testGetAllStations_AsNonAdmin_ReturnsForbidden() throws Exception {
    ...
    mockMvc.perform(get("/api/bar-stations")).andExpect(status().isForbidden());
  ...
  void testGetAllStations_Unauthenticated_ReturnsUnauthorized() throws Exception {
    ...
    mockMvc.perform(get("/api/bar-stations")).andExpect(status().isUnauthorized());
  ...
  void testGetUserStations_Success() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/bar-stations/user"))
        .andExpect(status().isOk())
  ...
  void testGetStationById_Success() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/bar-stations/1"))
        .andExpect(status().isOk())
  ...
  void testGetStationById_NotFound() throws Exception {
    ...
    // Act & Assert
    mockMvc.perform(get("/api/bar-stations/999")).andExpect(status().isNotFound());
  ...
  void testCreateStation_AsAdmin_Success() throws Exception {
    ...
    BarStationRequestDto request =
        new BarStationRequestDto(
            "New Station", "Test Description", true, List.of(userId1, userId2));

    // Arrange: Create expected response
    BarStationResponseDto response =
        createMockStationResponse(
            1L,
            1L,
            "New Station",
            true,
  ...
  void testCreateStation_AsNonAdmin_ReturnsForbidden() throws Exception {
    ...
    // Arrange: Create request DTO
    BarStationRequestDto request =
        new BarStationRequestDto("New Station", "Test Description", true, Collections.emptyList());
  ...
  void testCreateStation_DuplicateName_ReturnsConflict() throws Exception {
    ...
    BarStationRequestDto request =
        new BarStationRequestDto(
            "Duplicate Station", "Test Description", true, Collections.emptyList());
  ...
  void testUpdateStation_AsAdmin_Success() throws Exception {
    ...
    UUID userId = UUID.randomUUID();
    BarStationRequestDto request =
        new BarStationRequestDto("Updated Station", "Updated Description", false, List.of(userId));
  ...
  void testUpdateStation_AsNonAdmin_ReturnsForbidden() throws Exception {
    ...
    BarStationRequestDto request =
        new BarStationRequestDto(
            "Updated Station", "Updated Description", true, Collections.emptyList());
    ...
    // Verify service was NOT called
    verify(barStationService, never())
        .updateStation(anyLong(), anyLong(), any(BarStationRequestDto.class));
  }
  ...
  void testUpdateStation_DuplicateName_ReturnsConflict() throws Exception {
    ...
    BarStationRequestDto request =
        new BarStationRequestDto(
            "Duplicate Station", "Updated Description", true, Collections.emptyList());
  ...
  void testDeleteStation_AsAdmin_Success() throws Exception {
    ...
    mockMvc.perform(delete("/api/bar-stations/1")).andExpect(status().isNoContent());
  ...
  void testDeleteStation_AsNonAdmin_ReturnsForbidden() throws Exception {
    ...
    // Act & Assert: Non-admin should get 403 Forbidden
    mockMvc.perform(delete("/api/bar-stations/1")).andExpect(status().isForbidden());
  ...
  void testDeleteStation_NotFound() throws Exception {
    ...
    doThrow(new NotFoundException("Bar station not found"))
        .when(barStationService)
        .deleteStation(1L, 999L);

    // Act & Assert
    mockMvc.perform(delete("/api/bar-stations/999")).andExpect(status().isNotFound());
