...
class ProductServiceTest {
  ...
  void create_Success_CreatesInventoryAndTransaction() {
    ...
    saved.setBasePrice(BigDecimal.valueOf(5));
    saved.setActive(true);
    saved.setCreatedAt(OffsetDateTime.now());
    saved.setUpdatedAt(OffsetDateTime.now());
    when(productRepository.save(entity)).thenReturn(saved);
    when(productMapper.toResponse(saved))
        .thenReturn(
  ...
  void create_CategoryNotFound_Throws() {
    ProductRequestDto request =
        new ProductRequestDto("Beer", "Desc", BigDecimal.ONE, BigDecimal.ONE, BigDecimal.TEN, 9L);
    when(categoryRepository.findById(9L)).thenReturn(Optional.empty());
    ResponseStatusException ex =
        assertThrows(ResponseStatusException.class, () -> productService.create(request, 1L));
  ...
  void create_NameBlank_Throws() {
    ProductRequestDto request =
        new ProductRequestDto("   ", "Desc", BigDecimal.ONE, BigDecimal.ONE, BigDecimal.TEN, 7L);
    Category cat = new Category();
    cat.setId(7L);
    cat.setOrganizationId(1L);
    when(categoryRepository.findById(7L)).thenReturn(Optional.of(cat));
  ...
  void create_CategoryOrgMismatch_Throws() {
    ProductRequestDto request =
        new ProductRequestDto("Beer", "Desc", BigDecimal.ONE, BigDecimal.ONE, BigDecimal.TEN, 7L);
    Category cat = new Category();
    cat.setId(7L);
    cat.setOrganizationId(2L);
    when(categoryRepository.findById(7L)).thenReturn(Optional.of(cat));
    Product entity = new Product();
    entity.setName("Beer");
    when(productMapper.toEntity(request)).thenReturn(entity);
  ...
  void delete_NotFound_Throws() {
    ...
    ResponseStatusException ex =
        assertThrows(ResponseStatusException.class, () -> productService.delete(88L));
