...
class BarStationServiceTest {
  ...
  void createStation_Success_AssignsUsers() {
    ...
    BarStation saved =
        BarStation.builder().id(5L).name("Main").organizationId(1L).users(new HashSet<>()).build();
    when(barStationRepository.save(any(BarStation.class))).thenReturn(saved);
    when(barStationMapper.toResponseDto(saved))
        .thenReturn(new BarStationResponseDto(5L, 1L, "Main", "Desc", true, List.of(), null, null));
  ...
  void createStation_DuplicateName_Throws() {
    ...
    assertThrows(
        DuplicateResourceException.class, () -> barStationService.createStation(1L, request));
  }
  ...
  void updateStation_DuplicateName_Throws() {
    ...
    BarStationRequestDto request = new BarStationRequestDto("Main", null, null, null);
    assertThrows(
        DuplicateResourceException.class, () -> barStationService.updateStation(1L, 2L, request));
  }
  ...
  void getAllStations_ReturnsMappedList() {
    ...
    when(barStationMapper.toResponseDtoList(List.of(s1, s2)))
        .thenReturn(
            List.of(
  ...
  void createStation_DefaultsActive_WhenNoUsersProvided() {
    ...
    BarStation saved =
        BarStation.builder().id(3L).organizationId(1L).name("New").isActive(true).build();
    when(barStationRepository.save(any(BarStation.class))).thenReturn(saved);
    when(barStationMapper.toResponseDto(saved))
        .thenReturn(new BarStationResponseDto(3L, 1L, "New", null, true, List.of(), null, null));
  ...
  void updateStation_Success_AssignsUsers() {
    ...
    BarStation station =
        BarStation.builder()
            ...
            .users(new HashSet<>())
            .build();
    when(barStationRepository.findByOrganizationIdAndId(orgId, stationId))
    ...
    when(barStationMapper.toResponseDto(any()))
        .thenAnswer(
            a ->
                new BarStationResponseDto(
                    stationId, orgId, "Upd", null, false, List.of(), null, null));
  ...
  void getUserStations_ReturnsMapped() {
    ...
    when(userRepository.findById(uid)).thenReturn(Optional.of(user));
    when(barStationMapper.toResponseDtoList(anyList()))
        .thenReturn(
            List.of(new BarStationResponseDto(1L, 1L, "A", null, true, List.of(), null, null)));
