...
class InventoryServiceTest {
  ...
  void addStock_ProductInactive_ThrowsGone() {
    ...
    ResponseStatusException ex =
        assertThrows(
            ResponseStatusException.class, () -> inventoryService.addStock(request, userId, 1L));
  ...
  void removeStock_Insufficient_ThrowsBadRequest() {
    ...
    inv.setAdjustedPrice(BigDecimal.ONE);
    ...
    RemoveStockRequestDto request =
        new RemoveStockRequestDto(5L, BigDecimal.valueOf(5), null, null);
    ResponseStatusException ex =
        assertThrows(
            ResponseStatusException.class, () -> inventoryService.removeStock(request, userId, 1L));
  ...
  void adjustStock_Success_CreatesTransaction() {
    ...
    inv.setAdjustedPrice(BigDecimal.valueOf(2));
    inv.setUpdatedAt(OffsetDateTime.now());
    when(productRepository.findById(5L)).thenReturn(Optional.of(product));
    when(inventoryRepository.findByOrganizationIdAndProductId(1L, 5L)).thenReturn(Optional.of(inv));
    when(inventoryRepository.save(any(Inventory.class)))
  ...
  void getByOrganization_FiltersInactiveProducts() {
    ...
    when(inventoryMapper.toResponse(inv1))
        .thenReturn(
  ...
  void getByProductAndOrganization_ProductInactive_Gone() {
    ...
    inv.setUpdatedAt(OffsetDateTime.now());
    when(inventoryRepository.findByOrganizationIdAndProductId(1L, 10L))
  ...
  void getTransactionHistory_MapsUserInfo() {
    ...
    tx.setId(1L);
    tx.setInventory(inv);
    tx.setInventoryId(inv.getId());
    tx.setTransactionType("SALE");
    tx.setQuantityChange(new BigDecimal("-1"));
    tx.setQuantityBefore(BigDecimal.TEN);
    tx.setQuantityAfter(new BigDecimal("9"));
    tx.setPriceBefore(BigDecimal.ONE);
    tx.setPriceAfter(BigDecimal.ONE);
    tx.setReferenceId("ref");
    tx.setNotes("n");
    tx.setCreatedBy(uid);
    tx.setCreatedAt(OffsetDateTime.now());
    when(inventoryTransactionRepository.findByInventoryIdOrderByCreatedAtDesc(100L))
  ...
  void getUserSalesStats_ComputesCountsAndRevenue() {
    ...
    t1.setInventoryId(11L);
    t1.setTransactionType("SALE");
    t1.setReferenceId("o1");
    t1.setQuantityChange(new BigDecimal("-2"));
    t1.setCreatedBy(uid);
    t1.setBarStationId(stationId);
    InventoryTransaction t2 = new InventoryTransaction();
    t2.setInventoryId(12L);
    t2.setTransactionType("SALE");
    t2.setReferenceId("o2");
    t2.setQuantityChange(new BigDecimal("-1"));
    t2.setCreatedBy(uid);
    t2.setBarStationId(stationId);
    when(inventoryTransactionRepository.findSaleTransactionsByOrganizationId(orgId))
        .thenReturn(List.of(t1, t2));

    // inventories map to products with base prices
    Inventory inv1 = new Inventory();
    inv1.setId(11L);
    inv1.setProductId(101L);
    Inventory inv2 = new Inventory();
    inv2.setId(12L);
    inv2.setProductId(102L);
    ...
    Product p1 = new Product();
    p1.setId(101L);
    p1.setBasePrice(new BigDecimal("3.00"));
    Product p2 = new Product();
    p2.setId(102L);
    p2.setBasePrice(new BigDecimal("5.00"));
    ...
    user.setEmail("b@c.d");
    when(userRepository.findAllById(anyList())).thenReturn(List.of(user));
    BarStation station = new BarStation();
    station.setId(stationId);
    station.setName("Main");
